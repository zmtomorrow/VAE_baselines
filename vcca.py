...
class VCCA(nn.Module):
    ...
    def conditional_sample(self,y,iterations=1000):
        loss_list=[]
        self.eval()
...
if __name__ == '__main__':
    ...
    for i in range(0,opt['epochs']):
        loss=JointTrain(model,train_data_loader,opt)
    torch.save(model.state_dict(),'./save/Joint_epoch20.pth')


    model.load_state_dict(torch.load('./save/Joint_epoch20.pth'))
    ...
    sample, loss=model.conditional_sample(torch.tensor([3]))
    plt.plot(loss)
